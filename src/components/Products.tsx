...
function Products({ id, name, description,image, price, category, priceTakhfif} : IProduct ) {
    return (
        ...
        <div> 
            ...
            <div key={id} className="flex justify-center items-center relative z-30 mt-16 ">

                <Link href={`${id}`}>  <img src={image} className="w-52 flex  [filter:drop-shadow(0_3px_5px_rgba(0,0,0,0.5))] hover:scale-105 transition" /> </Link>

                <div className="flex justify-center items-center  rounded-2xl flex-col absolute top-46 bg-emerald-100 w-64 z-[-1]  [filter:drop-shadow(0_3px_5px_rgba(0,0,0,0.2))] hover:scale-105 transition matindark">
                    <Link href={`${id}`}>
